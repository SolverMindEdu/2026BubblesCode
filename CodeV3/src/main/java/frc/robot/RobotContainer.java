...
import frc.robot.io.OperatorControlsIO;
import frc.robot.io.OperatorControlsIOReal;
...
public class RobotContainer {
  ...
  private double driveScale = 0.52;
  ...
  private final OperatorControlsIO operatorIO =
    RobotBase.isReal() ? new OperatorControlsIOReal(1) : (inputs) -> {};

private final OperatorControlsIO.OperatorControlsIOInputs operatorInputs =
    new OperatorControlsIO.OperatorControlsIOInputs();
  ...
  private boolean isAligningForShot() {
    return driverInputs.rightTrigger > 0.5 && !isAimedWithinDegrees(2.0);
  }
  ...
  operatorIO.updateInputs(operatorInputs);
  }
...
private Command autoAimDrive() {
  ...
  private void configureBindings() {
    ...
    Trigger operatorY = new Trigger(() -> operatorInputs.y);
    Trigger operatorB = new Trigger(() -> operatorInputs.b);
    ...
    buttonY.onTrue( 
    Commands.runOnce(() -> {
      driveScale = (driveScale == 0.52) ? 0.65 : 0.52;
    })
  );

    operatorY.onTrue(
      Commands.runOnce(() -> {
        shooter.setTargetRps(49.5);
        hood.setTargetDegrees(3.0);
      }, hood, shooter)
  );

  operatorB.onTrue(
      Commands.runOnce(() -> {
        shooter.setTargetRps(62.0);
        hood.setTargetDegrees(23.0);
      }, hood, shooter)
  );
    ...
    }, hood, slapdown, rollers, indexer, kicker, shooterSubsystem)
);
